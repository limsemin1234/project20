...
class StockFragment : BaseFragment() {
    ...
    private fun updateImplementedFeatures() {
        val featuresText = """
            ...
            • 주식 매수/매도 시스템
            • 전체 매수/매도 기능
            • 수량 선택 버튼 (1, 5, 10, 20, 50, 100주)
            • 실시간 가격 변동
            • 손익 계산 및 표시
            • 평균 매입가 계산
            
            [주식 종목 시스템]
            • 대형주, 중형주, 소형주 총 30개 종목 중 랜덤 선택
            • 게임 시작 및 게임 오버 후 재시작 시 종목 랜덤 변경
            • 설정에서 게임 나가기로 종료 시 종목 유지
            • 각 종목명은 중복되지 않게 선택됨
            
            [가격 시스템]
            • 10,000원 ~ 200,000원 범위 내 10개 가격 중 랜덤 선택
            • 모든 종목의 가격이 중복되지 않도록 설정
            
            [기본 변동성]
            • 3만원 이하: 0.9 (안정적)
            • 3만원 초과 ~ 10만원 이하: 1.0 (보통)
            • 10만원 초과: 1.1 (변동이 큼)
            ...
            [추세 시스템]
            • 최근 가격 변동에 기반한 추세 분석
            • 추세 강도: -1.0 (강한 하락) ~ +1.0 (강한 상승)
            • 추세 영향력: 40%, 랜덤 변동: 60%
            
            [반동 메커니즘]
            • 3회 이상 동일 방향으로 연속 변동 시 반대 방향으로의 조정 확률 발생
            • 연속 변동 횟수가 늘어날수록 반동 확률 증가 (기본 30%에서 최대 100%까지)
            • 8회 이상 연속 같은 방향 변동 시 100% 확률로 강제 반동 발생
            • 반동 효과 발생 시 4회의 가격 변동 동안 지속적인 반대 방향 유도
            • 상승 반동: 하락 범위는 0.2배로 감소, 상승 범위는 2.5배로 증가
            • 하락 반동: 상승 범위는 0.2배로 감소, 하락 범위는 2.5배로 증가
            • 상승 반동 시 소형/중형/대형 호재 중 하나가 무작위로 적용
            • 하락 반동 시 소형/중형/대형 악재 중 하나가 무작위로 적용
            • 강제 반동 시에는 대형 호재/악재가 적용됨
            • 가격 이상치 감지 기능: 초기 가격의 2배 이상 또는 1/2 이하로 변동 시 강제 반동 발생
            • 강제 반동은 일반 반동보다 2배 길게 지속 (8회, 약 40초)
            • 반동 효과가 활성화된 동안에는 다른 이벤트의 영향을 받지 않음
            • 주식 시장의 평균 회귀(Mean Reversion) 성질 모델링
            ...
            • 시장 전체 이벤트
              - 경기 부양: 모든 종목 +0.02% ~ +0.03%
              - 경기 침체: 모든 종목 -0.03% ~ -0.02%
              - 시장 폭등: 모든 종목 +0.05% ~ +0.08%
              - 시장 폭락: 모든 종목 -0.08% ~ -0.05%
            ...
            [주의사항]
            • 한 종목에 여러 이벤트가 동시에 발생하면 변동폭이 큰 이벤트가 우선 적용됩니다.
            • 변동성 이벤트는 다른 이벤트와 함께 적용됩니다.
